...
TEST(TestAnalyseXML, TestCircle) {
    ...
    ASSERT_TRUE(
            result) << result.description(); // Si jamais result est faux, indique que le test est faux *et* affiche la string result.description() (qui contient la raison de l'erreur)
    Circle c(doc.child("Circle"));
    std::string c_dump_ref = R"(Circle "testCircle", x: 0, y: 1, r: 2, color: "Black"
)";
...
TEST(TestReadXML, TestGroupHybrid) {
    std::string s = R"(<?xml version = "1.0"?>
                    ...
                    <Group label="testGroup" x="5" y="6"/></Group>)";

    pugi::xml_document doc;
    pugi::xml_parse_result result = doc.load_string(s.c_str());
    ASSERT_TRUE(result) << result.description();
    Group g(doc.child("Group"));
    std::string g_dump_ref = R"(Group "testGroupHybrid", x: 0, y: 1, children: [
